...
![图片](https://uploader.shimo.im/f/E9YEPQhn8tI8RMIZ.png!thumbnail)
...
推荐多次检测直播失败时打开截图保存到桌面功。正常情况下保存到桌面的截图应该和下面这个图差不多，如果多次检测直播失败或者您保存到桌面的截图和下面的截图有差距的话，请点击自定义截图（自定义截图教程在上方），自己设置截图区域。   
